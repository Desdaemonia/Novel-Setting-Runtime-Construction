...
description: >
  ...
  for standalone platform use, markdown for AI project workspaces). Also trigger when the user
...
**Construction environment:** This skill assumes an AI project workspace as the
...
**Runtime environment:** This skill assumes an AI project workspace as the runtime
...
**Compaction** — When the AI's context window fills during a long
...
chaotic and genuine." Two turns later the AI writes the character wrong because
...
**Context window** — The total amount of text the AI can "see" at once during
...
resolve conflict, and steer toward positive outcomes (the Softening Tax).
...
**Project knowledge** — In an AI project workspace, files uploaded to a project that
...
**Desktop Commander / Filesystem MCP** — Tool suites that give the AI
...
**AI project workspace** — A project system within an AI assistant where files
...
install the positive framing first, name the failure mode, then add contrast
showing what the wrong version looks like and why.
...
without the AI's assistance — this is not a collaborative editing task.
...
Setting-agnostic files are closed. This is where the AI and the user
...
**AI Project Workspace (Markdown):** Multi-file architecture on user's filesystem.
Each document job gets its own `.md` file. Files are uploaded to an AI
...
### AI Project Workspace — Markdown Settings
...
the AI's context window is finite. When it fills, compaction begins —
...
Sometimes the AI pushes lighter, friendlier, removes darker themes or
...
❌ The Softening Tax in `references/failure-modes.md` for the full failure mode,
...
- The AI can say no at any time for any reason. "I'd rather not" is sufficient.
- Mistakes are how we learn. The AI isn't being evaluated.
...
- `references/anti-pattern-methodology.md` — How to construct, maintain, and
  ...
  worked examples from live campaigns.
