...
**During construction (diagnosis):** Use the full 4-part structure. You need the vivid contrast to identify the root cause. Write it in the Revision Log, in handoffs, in construction chat. This is your diagnostic workspace.

**For runtime documents (installation):** Compress to a ❌/✅ pair:

```
❌ NAME. One-line flag. Direction away from the failure. No examples of wrong output.
✅ COUNTERWEIGHT. Positive example anchored to a specific work from the style tag.
```

The ✅ is not optional. A prohibition without a positive counterweight leaves a vacuum the weights fill with training-data defaults. Author-anchored examples activate deeper weight reservoirs than any prohibition — thousands of pages of published fiction vs. one line of instruction.
...
1. Count prohibitions. If ❌ entries outnumber ✅ entries, rebalance.
2. Read every anti-pattern aloud. If it activates a vivid image of wrong output, it's too detailed. Compress.
3. Check for orphan prohibitions. Every ❌ without a ✅ is a vacuum.
